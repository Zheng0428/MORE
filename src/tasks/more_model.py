...
#        hid_dim = self.more_encoder.dim
        ...
        self.more_decoder.training = True
        # # VQA Answer heads
    ...
    def forward(self, feat, pos, sent, reward):
        """
        ...
        """
        x = self.more_encoder(sent, (feat, pos))
        reward = torch.round(torch.rand(32, 1))   #test
        seq = self.mix(x[0], x[1], reward, 767)   #mix the output of the lxmert and the reward 
